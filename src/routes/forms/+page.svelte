...
<div class="flex h-screen bg-slate-50 text-slate-900 font-sans overflow-hidden">
  ...
  <aside class="w-64 bg-white border-r flex flex-col p-4">
    <div class="font-black text-xl mb-6 px-2 italic tracking-tighter">Fabform</div>
    
    <button onclick={() => (isOpenModal = true)} class="w-full bg-slate-900 text-white py-2.5 rounded-xl font-bold hover:bg-black transition mb-4 shadow-sm">
      + New Form
    </button>
  ...
  <main class="flex-1 flex flex-col p-8 overflow-hidden">
    {#if selectedId}
      <header class="flex justify-between items-center mb-6">
        <div>
          <h1 class="text-3xl font-black tracking-tight">{activeForm?.name}</h1>
          <p class="text-slate-400 text-xs font-mono mt-1 uppercase">ID: {selectedId}</p>
        </div>
        <div class="flex gap-2">
          <a href={table.csvUrl} download="data.csv" class="bg-white border border-slate-200 px-5 py-2 rounded-xl text-sm font-bold hover:bg-slate-50 transition shadow-sm">Export</a>
          <button 
            disabled={!selectedRows.length} 
            onclick={() => (isOpenModal2 = true)}
            class="bg-red-50 text-red-600 px-5 py-2 rounded-xl text-sm font-bold disabled:opacity-30 hover:bg-red-100 transition"
          >
      ...
      <MenuBar id={selectedId} hideHomeButton={true} />
              ...
              <tbody>
                {#each table.rows as row (row[0])}
                  <tr class="border-b border-slate-50 hover:bg-slate-50 transition-colors">
                    <td class="p-4"><input type="checkbox" bind:group={selectedRows} value={row} /></td>
                    {#each row as cell}
    ...
    {:else}
      <div class="m-auto text-center border-2 border-dashed border-slate-200 rounded-[3rem] p-20">
        <p class="text-slate-300 font-black text-2xl uppercase tracking-tighter">Select a form to begin</p>
      </div>
    {/if}
  </main>
</div>

<Modal isOpenModal={isOpenModal} on:closeModal={() => (isOpenModal = false)} on:success={saveEndpoint} title="Create Form" buttonText="Save">
  <input bind:value={newFormName} placeholder="Form Name" class="w-full p-3 border border-slate-200 rounded-xl mt-4 outline-none focus:ring-2 focus:ring-slate-900" />
</Modal>
